...
<?php
	...
	if ($_SERVER['REQUEST_METHOD'] == 'POST') {
		...
		//Proceso de Update
		if(!empty($_POST['sucAnt'])){
			$sucursal = trim($_POST['sucAnt']);
			$sucursal = mysqli_real_escape_string($dbcon, $sucursal);

			$sucursalAct = trim($_POST['id_sucursal']);
			$sucursalAct = mysqli_real_escape_string($dbcon, $sucursalAct);
			...
			$query="select ActualizarSucursal('$sucursal','$sucursalAct','$nombre','$ciudad','$estado','$direccion','$apertura','$cierre','$telefono','$restaurante') as resp";
        ...
        echo '<h1>Atencion</h1>
        <p>La registro con ese identificador ya existe!</p><p><br /></p>';
			}else if($resp==0){
        echo '<h1>Atencion</h1>
        <p>No existe la sucursal!</p><p><br /></p>';
      }
		}
		else{
			//Proceso de Insert
	...
	<div class="contenedor">
		...
		<div class="contenedor col-md-3 center-block fondoazul">
			<form action="sucursalInsert.php" method="POST">
				<p>ID de la sucursal</p><input type="text" name="id_sucursal" required maxlength="7" value="<?php if (isset($_GET['id']))
          echo $_GET['id']; ?>"><br>
				<p>Nombre de la Sucursal</p><input type="text" name="nombre" required maxlength="40" value="<?php if (isset($_GET['nombre']))
          echo $_GET['nombre']; ?>"><br>
				<!-- AQUI DESPLEGAR LISTA DE CIUDADES Y ESTADOS -->
				<p>Estado:</p><input type="text" name="estado" required maxlength="40" value="<?php if (isset($_GET['provincia']))
          echo $_GET['provincia']; ?>"><br>
				<p>Ciudad:</p><input type="text" name="ciudad" required maxlength="40" value="<?php if (isset($_GET['ciudad']))
          echo $_GET['ciudad']; ?>"><br>
				<!-- ******************************** -->
				<p>Direccion:</p><input type="text" name="direccion" required maxlength="40" value="<?php if (isset($_GET['direccion']))
          echo $_GET['direccion']; ?>"><br>
				<p>Hora de apertura:</p><input type="text" name="h_apertura" required maxlength="10" value="<?php if (isset($_GET['apertura']))
          echo $_GET['apertura']; ?>"><br>
				<p>Hora de cierre:</p><input type="text" name="h_cierre" required maxlength="10" value="<?php if (isset($_GET['cierre']))
          echo $_GET['cierre']; ?>"><br>
				<p>Telefono:</p><input type="text" name="telefono" required maxlength="10" value="<?php if (isset($_GET['telefono']))
          echo $_GET['telefono']; ?>"><br>
				<!-- AQUI DESPLEGAR LISTA DE RESTAURANTES -->
				<p>ID del restaurante:</p><input type="text" name="id_restaurante" required maxlength="7" value="<?php if (isset($_GET['restaurante']))
          echo $_GET['restaurante']; ?>"><br>
				<!-- ******************************** -->
				<input type="text" name="sucAnt" required maxlength="7" value="<?php if (isset($_GET['id']))
          echo $_GET['id']; ?>">
